...
namespace Challenge.Valkimia.Infrastructure.Services
{
    ...
    internal class ApplicationUserService : IApplicationUserService
    {
        ...
        public async Task<Result> CreateUser(Cliente user, string password, List<string> roles, bool isActive)
        {
            ...
            try
            {
                ...
                applicationUser.UserName = user.Nombre + "." + user.Apellido;
                applicationUser.Lastname = user.Apellido;
                applicationUser.Name = user.Nombre;
        ...
        public async Task<Result<IList<Cliente>>> GetAllUsers(QueryOptions options = null)
        {
        ...
        public async Task<Result<Cliente>> GetUser(Guid id)
        {
            var serviceResult = new Result<Cliente>();
            try
            {
                serviceResult.Data = await _mapper.ProjectTo<Cliente>(GetUsersWithRoles()).FirstOrDefaultAsync(u => u.Id == id);
                serviceResult.Successful();
            }
            catch (Exception ex)
            {
